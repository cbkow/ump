...
bool D3D11PlayerRenderer::init(PlayerWindow *window)
{
    ...
    // The thumbnail pool is normally lazy-init'd by TimelineThumbnailCache
    // on first upload, but the viewport-notice card (ARRIRAW / unsupported
    // media) also lives in it and can be needed before any timeline thumb
    // exists. Initialize up front (idempotent; 256 MB to match the cache).
    if (!D3D11TexturePool::thumbnailInstance().isInitialized()) {
        D3D11TexturePool::thumbnailInstance().initialize(256ULL * 1024 * 1024);
    }
