...
private:
    ...
    // Per-side display rotation in quarter-turns CW {0..3}
    // (setRotationA/B convert from degrees). Swaps the effective fit
    // dims for odd quarters; the compositor MSL inverse-rotates its
    // sampling. Same GUI-sets / render-reads idiom as PAR.
