...
u.m.p. supports several pipeline modes with various media types. What this means in practice: When a particular pipeline mode is selected, our FBOs are recreated (RGBA9, RGBA16, RGBA16F`), and the cache settings are adjusted accordingly. Here is a breakdown:
...
- **HDR Passthrough:** doesn't work yet, but its it will be used to passthrough HDR10 PQ (like HVEC) from MPV unaltered when in Windows HDR mode (if I ever get mpv to play along.)
